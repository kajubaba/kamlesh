...
import com.narendra.sams.web.restws.fee.vo.AnnualFeeReportVO;
...
@Controller
@RequestMapping({"/ws/fee/report"})
public class AnnualFeeReportRestController {
    @Autowired
    ...
    public List<AnnualFeeReportVO> getAnnualReport(Long academicYearId, Long studentStatus) {
        if (academicYearId == null) {
            academicYearId = UserSessionManager.getUserSession(this.webApplicationContext).getAcademicYearOfAdmission().getId();
        }
        return prepareAnnualFeeReportVO(this.feeReportService.getAnnualFeeReport(academicYearId, studentStatus));
    }

    private List<AnnualFeeReportVO> prepareAnnualFeeReportVO(List<CourseYearFeeSummary> classFees) {
        List<AnnualFeeReportVO> annualFeeReportVOs = new ArrayList();
        if (!(classFees == null || classFees.isEmpty())) {
            for (CourseYearFeeSummary courseYearFeeSummary : classFees) {
                AnnualFeeReportVO annualFeeReportVO = new AnnualFeeReportVO();
                if (courseYearFeeSummary.getCourse().getDuration().shortValue() == (short) 1) {
                    annualFeeReportVO.setClassName(courseYearFeeSummary.getCourse().getDisplayName());
                } else {
                    annualFeeReportVO.setClassName(new StringBuilder(String.valueOf(courseYearFeeSummary.getCourse().getDisplayName())).append(" , ").append(courseYearFeeSummary.getCourseYear().getName()).append(" Yr.").toString());
                }
                annualFeeReportVO.setStudentCount(courseYearFeeSummary.studentCount());
                annualFeeReportVO.setProjectedFee(courseYearFeeSummary.getProjectedFee() + courseYearFeeSummary.getBusFee());
                annualFeeReportVO.setDiscount(courseYearFeeSummary.getDiscoutGiven());
                annualFeeReportVO.setActualPayable((courseYearFeeSummary.getProjectedFee() + courseYearFeeSummary.getBusFee()) - courseYearFeeSummary.getDiscoutGiven());
                annualFeeReportVO.setPaidFee(courseYearFeeSummary.getDepositedFee());
                annualFeeReportVO.setPaidLateFee(courseYearFeeSummary.getLateFeeDeposited());
                annualFeeReportVO.setUnpaidFee((annualFeeReportVO.getProjectedFee() - annualFeeReportVO.getDiscount()) - annualFeeReportVO.getPaidFee());
                annualFeeReportVOs.add(annualFeeReportVO);
            }
        }
        return annualFeeReportVOs;
    }
