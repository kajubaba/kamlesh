...
samsApp.controller('feehead.list.controller', function(feeHeadService, serverErrorHandlerService, $scope, $route, $rootScope, $location, $routeParams, blockUI){
	...
	$scope.gridOptions = {
	        ...
	        columnDefs: [
	          { 
	        	  ...
	        	  cellTemplate: '<div class="ui-grid-cell-contents" ><a href="#/admin/feehead/{{row.entity.id}}">{{row.entity.name}}</a></div>'
	          },
	...
	this.getFeeHeads = function(){
		feeHeadService.getFeeHeads(this.getFeeHeadsSuccess, serverErrorHandlerService.handleError);
	}
	
	this.getFeeHeadsSuccess = function(response, status){
