...
samsApp.controller('studentcategory.controller', function(studentCategoryService, serverErrorHandlerService, $scope, $route, $rootScope, $location, $routeParams, blockUI){
	...
	$scope.gridOptions = {
	        ...
	        columnDefs: [
	          { 
	        	  ...
	        	  cellTemplate: '<div class="ui-grid-cell-contents" ><a href="#/admin/studentcategory/{{row.entity.id}}">{{row.entity.name}}</a></div>'
	          },
	...
	this.getStudentCategories = function(){
		studentCategoryService.getStudentCategories(this.getStudentCategoriesSuccess, serverErrorHandlerService.handleError);
	}
	
	this.getStudentCategoriesSuccess = function(response, status){
