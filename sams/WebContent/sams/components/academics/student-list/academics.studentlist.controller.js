...
samsApp.controller('sams.academics.studentlist.controller', function(studentListService, $routeParams, $scope, blockUI){
	...
	this.populateClassDropDown = function(response){
		...
		if(response.length > 0){
			...
			$.each(response, function(i)
			{
			    ...
			    var a = $('<a/>')
			        .attr("href", "#/academics/classwise/"+$scope.selectedStudentStatus+"/"+ response[i].academicYearClassId+"/"+$scope.selectedAcademicYear)
	...
	this.getActiveClassesSuccess = function(response, status){
		...
		if(response.length > 0){
			...
			$.each(response, function(i)
			{
			    ...
			    var a = $('<a/>')
			        .attr("href", "#/academics/classwise/"+$scope.selectedStudentStatus+"/"+ response[i].classId)
	...
	this.filterClasswiseAdmissionCount = function(academicYearId){
		window.location ="#academics/classview/"+$scope.selectedStudentStatus+"/"+academicYearId;
	}
	
	this.filterAdmissions = function(academicYearId){
		window.location ="#academics/studentview/"+$scope.selectedStudentStatus+"/"+academicYearId;
	}
